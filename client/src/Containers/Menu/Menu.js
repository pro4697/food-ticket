...
import { FadeIn, BgColor } from '../../common/Style_Etc';
		...
		<StyledRow gutter={[32, 32]} justify='center'>
			{Menu.map((item, idx) => (
				<StyledCol xl={4} lg={6} md={6} sm={8} xs={12} key={idx}>
					<Item onClick={onClick} value={idx} color={BgColor[idx % BgColor.length]}>
						<Img draggable='false' src={`${SERVER}/${item.url}`} alt={item.name} />
						<ItemInfo>
