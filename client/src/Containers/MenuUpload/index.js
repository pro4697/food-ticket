	...
	const onSubmit = (e) => {
		...
		axios.post(`${SERVER}/api/menus/uploadfiles`, formData).then((response) => {
			if (response.data.success) {
				...
				axios.post(`${SERVER}/api/menus/savefiles`, variable).then((response) => {
					if (response.data.success) {
	...
	return (
		<App>
			...
			<Form required onSubmit={onSubmit}>
				<Dropzone onDrop={onDrop} accept='image/*' multiple={false} maxSize={3072000}>
					{({ getRootProps, getInputProps }) => (
						<DropBox {...getRootProps()}>
							<input {...getInputProps()} />
							{Preview && <img src={Preview} alt='' style={{ height: '100%' }} />}
